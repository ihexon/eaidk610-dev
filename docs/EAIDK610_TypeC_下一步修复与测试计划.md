...
- 使用 GitHub 托管的原生 ARM64 `ubuntu-24.04-arm`，不把 Docker 执行容器或 `.166` 注册为自托管 runner；构建脚本同时核对 `uname -m=aarch64` 和 Debian `arm64` 架构。[runner 说明](https://docs.github.com/en/actions/how-tos/write-workflows/choose-where-workflows-run/choose-the-runner-for-a-job)
