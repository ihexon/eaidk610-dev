...
- the upstream `rk3399-eaidk-610.dtb` plus the tested Type-C overlay;
...
Type-C properties before creating a GitHub pre-release.

```sh
git submodule update --init --recursive
scripts/preflight-image-build.sh --network
gh workflow run build-armbian-image.yml \
  --repo ihexon/eaidk610-dev \
  --ref main \
  -f release_tag=eaidk610-armbian-edge-r1
```

The workflow never connects to a board or flashes eMMC. Hardware deployment is
a separate explicit operation after offline image checks pass.
